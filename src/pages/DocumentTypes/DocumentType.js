...
const DocumentType = () => {
    ...
    const documentTypeName = [
        'Счёт-фактура', 'Договор', 'Договор', 'Акт'
    ]
    ...
    useEffect(() => {
        const fetchApi = async () => {
            const res = await documentTypeServices.getAllDocumentType(page, Number(limit));
    ...
    const handleDelete = async (id) => {
        const confirmMsg = 'Вы уверены, что хотите безвозвратно удалить этот документ?';
    ...
    const handleDeleteMany = async () => {
        const confirmMsg = 'Вы уверены, что хотите безвозвратно удалить эти документы?';
        if (!window.confirm(confirmMsg)) return;
        const data = {
            arrayId: checked,
        };
    ...
    return (
        <>
            ...
            <div className="flex flex-col md:flex-row items-center md:justify-between bg-[#f7f7f7] p-[16px] border border-solid border-[#cccccc] mb-[12px] md:mb-0 shadow-4Way">
                <h1 className="text-[1.8rem] md:text-[2.4rem] font-bold">Тип документа</h1>
                ...
                <div className="flex items-center justify-between py-3 mx-5">
                    ...
                    <div className="flex items-center">
                        <p className="text-[1.5rem] mr-9">
                            Показать <span>{documentTypeLists?.length === 0 ? 0 : rowStart}</span> от{' '}
                            <span>{rowEnd + documentTypeLists?.length}</span> до{' '}
                            <span>{allDocumentTypes?.length}</span> элементов
                        </p>
                        <div
