...
const DocumentDetail = () => {
    ...
    useEffect(() => {
        const setStatusPercentage = () => {
            if (document.status === 'Завершено') {
                setPercent('100%');
                setStatusStyle('text-[1rem] md:text-[1.4rem] progress-bar full');
            } else if (document.status === 'В работе') {
                setPercent('60%');
    ...
    return (
        <div className="bg-white p-[16px] mb-5 shadow-4Way">
            <h1 className="text-[2.2rem] font-bold">Подробности</h1>
            <div className="mt-12">
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Название:</h3>
                    <p className="flex-1">{document?.documentName}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Тип документа:</h3>
                    <p className="flex-1">{document?.type}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Размер:</h3>
                    <p className="flex-1">{document?.code}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Дата создания:</h3>
                    <p className="flex-1">{document?.sendDate}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Где создан:</h3>
                    <p className="flex-1">{document?.sender}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Уровень важности:</h3>
                    <div className="flex-1">
                        <p className={setLevelColor(document?.level)}>{document?.level}</p>
                    </div>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Статус:</h3>
                ...
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Отдел:</h3>
                    <p className="flex-1">{document?.currentLocation}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Описание:</h3>
                    <p className="flex-1">{document?.note}</p>
                </div>
                <div className="flex flex-col md:flex-row my-5">
                    <h3 className="w-[180px] font-bold">Вложения:</h3>
                    <ul className="flex-1">
