...
const DocumentIn = () => {
    ...
    const [allDocuments, setAllDocuments] = useState(['1', '1', '1', '1', '1', '1', '1', '1', '1']);
    const [documentLists, setDocumentLists] = useState(['1', '1', '1', '1', '1', '1', '1', '1', '1']);
    ...
    const statusOptions = ['Инициализация', 'В работе', 'Ожидание утверждения', 'Завершено'];
    ...
    const code = ["1424", "322", "453", "451", "565", "362", "1456"];
    const documentName = ["Договор аренды помещения", "Устав организации", "Счет на оплату услуг", "Заказ на поставку оборудования", "Акт приема-передачи материалов", "Отчет по выполненным работам за месяц", "Документ"];
    const type = ["PDF", "Docx", "xlsx", "PDF", "txt", "Docx", "PDF", "xlsx", "Docs"];
    const level = ["Срочный", "Несрочный", "Несрочный", "Срочный", "Срочный", "Сверх срочный", "Несрочный"];
    ...
    const handleDelete = async (id) => {
        const confirmMsg = 'Вы уверены, что хотите удалить выбранный документ?';
    ...
    const handleDeleteMany = async () => {
        const confirmMsg = 'Вы уверены, что хотите удалить выбранные документы?';
        if (!window.confirm(confirmMsg)) return;
        const data = {
            arrayId: checked,
        };
    ...
    return (
        <>
            <div className="bg-white p-[16px] mb-5 shadow-4Way">
                ...
                <div className="flex flex-col md:flex-row gap-5 mt-5">
                    <div className="flex-1">
                        <label className="text-[1.4rem]">Название:</label>
                        <InputField className="default" placeholder="Название" value={fName} setValue={setFName} />
                    </div>
                    <div className="flex-1">
                        <label className="text-[1.4rem]">Тип документов:</label>
                        <InputField className="default" placeholder="Тип документов" value={fCode} setValue={setFCode} />
                    </div>
                    <div className="flex-1">
                        <label className="text-[1.4rem]">Дата создания:</label>
                        <InputField
                            name="date"
                ...
                <div className="flex flex-col md:flex-row gap-5 mt-[12.5px]">
                    <div className="flex-1">
                        <label className="text-[1.4rem]">Формат:</label>
                        <DropList
                            selectedValue={fType}
            ...
            <div className="flex flex-col md:flex-row items-center md:justify-between bg-[#f7f7f7] p-[16px] border border-solid border-[#cccccc] mb-[12px] md:mb-0 shadow-4Way">
                <h1 className="text-[1.8rem] md:text-[2.4rem] font-bold">Список документов</h1>
                <div
                    className={
                        userRole === 'Member'
                            ? 'hidden'
                                            ...
                                            return (
                                                <tr key={index} className="border-b dark:border-neutral-500">
                                                    ...
                                                    <td title={dcl?.level} className="whitespace-nowrap px-6 py-4">
                                                        <div className={setLevelColor(level[index])}>{level[index]}</div>
                                                    </td>
                                                    <td
                                                        title={dcl?.status}
                                                        className={
                ...
                <div className="flex items-center justify-between py-3 mx-5">
                    ...
                    <div className="flex items-center">
                        <p className="text-[1.5rem] mr-9">
                            Показать <span>{documentLists?.length === 0 ? 0 : rowStart}</span> по{' '}
                            <span>{rowEnd + documentLists?.length}</span> из <span>{allDocuments?.length}</span> элементов
                        </p>
                        <div
